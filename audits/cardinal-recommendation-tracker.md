...
| GA4 key-event hygiene (supports the same recommendation) | ❌ **found 2026-07-22** | GA4 has **13 key events, all intent-level clicks, zero booked-level** (incl. `i_am_existing_patient_click`, `get_directions`, `email_click`). After the two Liine booked events are marked key, review demoting the noisiest intent events with Blue Ox (affects any Ads conversion imports) so GA4 conversion counts mean patients, not clicks. |
