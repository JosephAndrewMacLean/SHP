# Spine Page Execution List v3 — BY URL: Opportunity → Meta → On-Page Content Updates

**Executor:** Randall (metas + plugin-guided copy) + content-creator drafts + Paul (modules/dev)
**Approver chain:** Joe → Mitch (clinical) → compliance · **SHARE WITH CARDINAL BEFORE EXECUTION**
**Status: DRAFT — gated items blocked until cleared** · ⚠ = confirm exact live URL in CMS first
**Format per URL:** the NP opportunity → Cardinal-audit finding it serves → meta (title ≤60 /
description ≤155) → **the on-page content updates required to attain the opportunity** → gate.
Template references = `playbooks/spine-page-templates.md`.
...
condition hubs rescue · zero decision-support · symptom-entry gap · non-branded gap ·
E-E-A-T/YMYL. Rules: no stats/superlatives/guarantees; ⏳ACCESS wording needs Katie/Kelly;
**DO-NOT-TOUCH titles:** Salar bio · neck-fracture · caudal-esi · MILD (desc/content only);
every edit logged; GSC per URL at 1/2/3/4 weeks.

---

# TIER 1 — proven demand + live supply

### `/` — Homepage
**Opportunity:** highest-visibility snippet on the site is empty (audit: missing meta).
**Meta:** *(title: confirm brand title)* · `Orthopedic and spine care across metro Detroit — 8 locations, same-week appointments, and complete care from diagnosis to recovery. Book online.` ⏳ACCESS
...
**Meta:** `Spine & Back Pain Specialists in Sterling Heights, MI` · `Spine clinics five days a week in Sterling Heights — including Fridays. Surgeons and pain specialists in one office. Book online or call.` ⏳ACCESS
**Content updates (template §3.2):** 1) **roster-by-day module** (single-source dataset; spine: Varghese Wed/Fri, Maslak Mon, Munk Wed AM/Fri, McCarty Wed/Thu, Salar Tue + ortho/hand/pain rows); 2) H1 = the title promise; 3) book-online-first + one tracked number; 4) insurance snippet (varies-by-provider link, no flat claims); 5) local reviews slot (Rater8 Lane-D screened); 6) paid-LP variant (stripped nav) for the SH campaigns; 7) near-you cross-links (Livonia/Troy).
**Gate:** Katie schedule confirm · ⏳ACCESS.
...
**Meta:** `Orthopedic Urgent Care — Walk In, See a Specialist Fast` · `Injured? Walk in and see an orthopedic specialist — most patients get X-rays the same visit. Faster and more specific than the ER for bone and joint.`
**Content updates:** 1) unique H1; 2) what-to-bring / what-happens block; 3) locations + hours module (single-source); 4) source-or-remove the "specialist in 20 min / 80% X-ray" stats (V2 class); 5) link from hub symptom router + location pages.
**Gate:** V2 (stats) · Cardinal (parameter indexing fix).

---

# TIER 2 — growth unlocks (gate per URL)

### ⚠ Troy location page
**Opportunity:** the Oakland unlock — spine Mon–Thu supply, ~4,800 obtainable patients/yr, zero current presence; Rochester Hills geography included.
**Meta:** `Spine Specialists in Troy, MI — Rochester Rd Clinic` · `Spine care in Troy Monday–Thursday, serving Troy and Rochester Hills. Neurosurgical and orthopedic spine specialists in one clinic. Book online.`
**Content updates:** 1) full §3.2 rebuild, spine-first roster module (§1 table of the schedule work order); 2) Rochester Rd / Rochester Hills service-area copy (no separate Rochester clinic — corrected 7/21); 3) "new patients welcome" + book-online-first; 4) Zamorano/Salar/Munk/Mayo/Sorensen cards; 5) LP variant for the future Cardinal campaign. **GBP excluded (separation rule).**
**Gate:** Katie confirm · V5 (Zamorano) · sequenced after SH/Livonia.
...
**Opportunity:** the #1 differentiation tryout (Maslak); copy exists buried on `/treatment/microdiscectomy/` (0 rank).
**Meta:** `Endoscopic Spine Surgery in Michigan — Minimally Invasive` · desc per v2.
...
**Content updates:** §3.2 hyperlocal variant — city-first opening with real proof points (distance/minutes, that clinic's spine team + days), canonical self-referential (Paul), route to nearest clinic; NO thin doorway copy — if the city can't get unique local proof, it doesn't ship.
**Gate:** **D3 list** · Katie · V6 precedes any satellite.

---

# TIER 3 — supporting / hygiene

### ⚠ Southfield location page
**Meta:** per v2 (podiatry-led). **Content:** roster module led by podiatry (incl. **home-visit podiatry callout**); honest spine line ("select Wednesdays — or same-week in Livonia, 20 min away" ⏳ACCESS); Mazor X claim migrates to hub (local mention stays).
**Gate:** ⏳ACCESS · Katie.
...
**Meta:** per v2. **Content:** fix geography (SH + Port Huron + Harbor Beach outreach — NOT Saginaw/Midland); collegiate-team-physician story; days line; sports links (Phase 2 seed).
...
**Opportunity:** audit's wrong-template class, confirmed live (carries a carpal-tunnel title).
**Meta:** `Broken Ankle: Symptoms, Treatment & Recovery Time` · desc per v2. **Content:** verify the body matches the topic (wrong-template check); urgent-care cross-link.
**Gate:** —
...
3. **⚠ sweep:** Randall confirms exact live URLs in the CMS before any edit.
4. **Measurement:** GSC per URL, 1/2/3/4-week OODA; audit KPIs: CTR 1.15% → 3–5% benchmark,
   non-branded share up from ~20%, elective-funnel entries converting (Liine-tagged).
