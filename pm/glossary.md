...
| **Home route day (K-R01…)** | The **geography**: a coordinate-clustered set of ~10 accounts that belong together on a map (re-clustered Aug 7 on exact MMC coordinates; ex-Sean combined into Kristen's days; same-building accounts atomic). It's the territory reference and the MMC `Route Day` field — you drive *scheduled days*, not home days. |
| **Scheduled day (K-W2D1)** | The **calendar**: one actual day of work — Kristen, week 2, day 1. ~10 stops in one area, built from whoever is DUE that week (T1 21d · T2 30d · T3 45d — the clock runs on actual MMC check-ins since Aug 7), same-building practices kept together, prospects riding along, plus 2 finish-early bonus stops. One Weekly Plan row = one drive; the *Stops by Day* tab lists the same days stop-by-stop. |
