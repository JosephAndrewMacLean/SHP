...
| **Home route day (K-R01…K-R22)** | The **geography**: a coordinate-clustered set of ~10 accounts that belong together on a map. It's the territory reference (Route Days tab in each workbook) and the value in MMC's `Route Day` field. You don't drive a home day as such — you drive scheduled days. |
| **Scheduled day (K-W1D1)** | The **calendar**: one actual day of work — Kristen, week 1, day 1. ~10 stops in one area, built from whoever is due that week (T1 21d · T2 30d · T3 45d) with prospects from the same area riding along to fill it. One row on the Weekly Plan = one drive; the Stops column is the whole day. Each stop lists its home route day so you can cross-reference. Replaced the stitched `R11+R12` labels (Jul 31, after Kristen read one as a 19-stop day). |
